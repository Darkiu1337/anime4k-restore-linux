...
# comments). Prefer a deployed copy, else the repo one.
ak_preset_json() {
  if [ -f "$ANIME4K_SHADER_DIR/presets.json" ]; then
    printf '%s' "$ANIME4K_SHADER_DIR/presets.json"
  else
    printf '%s' "$ANIME4K_ROOT/shaders/presets.json"
  fi
}
...
ak_render_preset_conf() { # <preset> <conf> <shader_dir> [fallback_shader_dir]
  local preset="$1" conf="$2" shader_dir="$3" fallback="${4:-$ANIME4K_ROOT/shaders}" manifest
  manifest="$(ak_preset_json)"
  python3 - "$manifest" "$preset" "$conf" "$shader_dir" "$fallback" <<'EOF'
import json, os, sys
manifest, preset, conf, shader_dir, fallback = sys.argv[1:6]
try:
...
lines = ["# generated by anime4k — Clear preset: %s" % preset,
...
for name, fname in (p.get("shaders") or {}).items():
    if os.path.isabs(fname):
        ...
        # Prefer the active shader dir; fall back to the repo copy so a dev
        # checkout works before install.sh redeploys the new files.
        path = os.path.join(shader_dir, fname)
        if not os.path.exists(path):
            path = os.path.join(fallback, fname)
...
ak_vkbasalt_env() {
  ...
  if ak_is_preset "$variant"; then
    ak_render_preset_conf "$variant" "$conf" "$ANIME4K_SHADER_DIR" \
